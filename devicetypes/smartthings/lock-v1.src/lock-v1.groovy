/**
*02/23/2016  if(result_name  == "lock") {
*12/30/2015  lock and unlock status
*12/28/2015  lock and unlock status
 ...
 */
metadata {
  definition (name: "lock_v1", namespace: "smartthings", author: "SmartThings") {
    capability "Actuator"
    capability "Lock"
    capability "Polling"
    capability "Refresh"
...
def parse(String description) {
  ...
  ///send to hook 12/28/2015 
    def debug = false
    def i = null
    try {
      i = result.data.usedCode
    } catch (e) {
    }
    def result_name = ''
    try {
 ...
 if(result_name  == "lock") {
        ...
        def body =  [
            deviceId: "${result.linkText}",
            slot: "${i}",
            state: "${result.value}"
        ]
        def params1 = [
            uri: "https://secure.rently.com/devices/hook",
            body: body 
        ]
        try {
        ...
        if(debug)
        {
            log.debug "testing degbug-----------------------------------"
            def params3 = [
                uri: "http://requestb.in/1mley5k1",
                body: body 
            ]

            try {
                httpPostJson(params3) { resp ->
                    resp.headers.each {
                        log.debug "${it.name} : ${it.value}"
                    }
                    log.debug "response contentType: ${resp.    contentType}"
...
def zwaveEvent(UserCodeReport cmd) {
    ...
    (cmd.userIdStatus == UserCodeReport.USER_ID_STATUS_STATUS_NOT_AVAILABLE && cmd.user && code != "**********"))
  {
    ...
    if (!code && cmd.userIdStatus == 1) {  // kwikset sends blank code to notify of a changed code 
